...
(function() {
	...
	var showAssessment = function() {
		// go to the assessment screen
		screenManager.showScreen('assessment');
	}
	
	var updateList = function(sortOn) {
		var data = traineeListData;
		//alert('traineeListData: '+traineeListData)
		// clear the current table data
		$('table.assessment-list tr').each(function(index) {
		...
		for(var i=0; i < data.length; i++) {
			...
			if(data[i][ind_isChecklist] != 'false') continue;
			if(currentGroup.id) {
				// filter by group
			...
			toInsert += '<td class="name"><a class="show-assessment" href="#">Stage&nbsp;'+ data[i][ind_stage]+' &nbsp;- &nbsp;'+data[i][ind_docname]+'</a></td>';
			...
			$('table.assessment-list').append(toInsert);
		};
		
		// set up the click handlers 
		$('table.assessment-list td.name a').click(function(e) {
			...
			currentAssessment = {id:docID, name:docName, stage:docStage, stageName:docStageName};
