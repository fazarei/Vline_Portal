...
(function() {
	var toggleRoleCheckbox = function(cb) {
		...
		var userID = parentTR.attr('data-userid');
	...
	var updateList = function(data, sortOn) {
		...
		for(var i=0; i < data.length; i++) {
			...
			$('table.trainer-list td.name').each(function(index) {
			 ...
			 $('table.trainer-list').attr('id','trainer-list');	
			if(!row) {
				var toInsert = '<tr data-userid="'+data[i][0]+'">';
		...
		var $rows = $('.trainer-list tbody tr');
			...
			"Submit": function() {
				...
				$(this).find('.user-list tr').each(function(index) {
					...
					if($(this).find('td.add a').hasClass('checked')) {
						...
						var toInsert = '<tr data-userid="'+userID+'">';
						...
						$('tr[data-userid="'+userID+'"] .checkbox').click(function(e) {
							e.preventDefault(); if($(this).hasClass('disabled')) return; 
							
							toggleRoleCheckbox($(this));
						});
								// send the data to insert
								$.ajax({
									url: '../getinfo.php',
									type: 'POST',
									data: {mode: 'insertAssessorTrainer', data: userName}
									})
									.done(function( text ) {
